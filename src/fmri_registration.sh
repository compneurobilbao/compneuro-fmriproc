...
timepoint=$(date +"%H:%M")
echo "$timepoint    **Doing transformation between functional and anatomical images...**" >> /project/log/fMRIpreproc_${timestamp_initial}.txt
...
timepoint=$(date +"%H:%M")
echo "$timepoint    **Doing transformation between standard and anatomical images...**" >> /project/log/fMRIpreproc_${timestamp_initial}.txt
